...
Function Show-HelpConsole {
	...
	$HelpListBox.TabIndex = 2
		...
		$remove4Default = $newline+$newline+$newline+'      ## REMEBER TO REMOVE ITEMS FROM THE DEFAUL USER PROFILE IN THE UNINSTALL SECTION!'+$newline+$newline+'        Remove-Folder -Path "${Env:SystemDrive}\Users\Default\AppData\Roaming\StorageExplorer"'
		$AllUsersText = $AllUsersDescription+$AllUsersLogFolder+$remove4Default
		
	$HelpListBox.add_SelectedIndexChanged({if($HelpListBox.SelectedItem -eq 'Copy files / folder to all users'){$HelpTextBox.Text = "$AllUsersText" | Out-String}elseif($HelpListBox.SelectedItem -eq 'Variables'){$HelpTextBox.Text = "$VariablesText" | Out-String}else{$HelpTextBox.Text = Get-Help -Name $HelpListBox.SelectedItem -Full | Out-String}})
	
	$helpFunctions = Get-Command -CommandType 'Function' | Where-Object { ($_.HelpUri -match 'powershellgallery') -and ($_.Definition -notmatch 'internal script function') } | Select-Object -ExpandProperty Name
